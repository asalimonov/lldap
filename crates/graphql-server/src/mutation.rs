...
impl<Handler: BackendHandler> Mutation<Handler> {
    ...
    async fn add_user_to_group(
        ...
        if context.validation_result.is_user_manager() && !context.validation_result.is_admin() {
            ...
            if user_groups.iter().any(|g| g.display_name == "lldap_admin".into() || g.display_name == "lldap_user_manager".into()) {
                span.in_scope(|| debug!("UserManager cannot add a user to admin or user_manager groups"));
                return Err("UserManager cannot add a user to admin or user_manager groups".into());
            }
            // UserManager cannot add users to admin or user_manager groups
            if group_id == 1 || group_id == 4 {
                span.in_scope(|| debug!("UserManager cannot add users to admin or user_manager groups"));
                return Err("UserManager cannot add users to admin or user_manager groups".into());
            }
        }
        handler
    ...
    async fn remove_user_from_group(
        ...
        if context.validation_result.is_user_manager() && !context.validation_result.is_admin() {
            let user_groups = handler.get_user_groups(&user_id).await?;
            if user_groups.iter().any(|g| g.display_name == "lldap_admin".into() || g.display_name == "lldap_user_manager".into()) {
                span.in_scope(|| debug!("UserManager cannot modify admin or user_manager users"));
                return Err("UserManager cannot modify admin or user_manager users".into());
            }
            // UserManager cannot remove users from admin or user_manager groups
            if group_id == 1 || group_id == 4 {
                span.in_scope(|| debug!("UserManager cannot remove users from admin or user_manager groups"));
                return Err("UserManager cannot remove users from admin or user_manager groups".into());
            }
        }
        handler
    ...
    async fn delete_user(context: &Context<Handler>, user_id: String) -> FieldResult<Success> {
        ...
        if context.validation_result.is_user_manager() && !context.validation_result.is_admin() {
            let user_groups = handler.get_user_groups(&user_id).await?;
            if user_groups.iter().any(|g| g.display_name == "lldap_admin".into() || g.display_name == "lldap_user_manager".into()) {
                span.in_scope(|| debug!("UserManager cannot delete admin or user_manager users"));
                return Err("UserManager cannot delete admin or user_manager users".into());
            }
        }
