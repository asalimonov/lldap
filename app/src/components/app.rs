...
impl Component for App {
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        html! {
          <div>
            <Banner is_admin={is_admin} is_user_manager={is_user_manager} username={username} on_logged_out={link.callback(|_| Msg::Logout)} />
            <div class="container py-3 bg-kug">
              <div class="row justify-content-center" style="padding-bottom: 80px;">
                <main class="py-3">
                  <Switch<AppRoute>
                    render={Switch::render(move |routes| Self::dispatch_route(routes, &link, is_admin, is_user_manager, password_reset_enabled))}
                  />
                </main>
              </div>
              {self.view_footer()}
